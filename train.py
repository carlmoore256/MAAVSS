...
if __name__ == "__main__":
    ...
    with wandb.init(project='AV-Fusion-AVSE', entity='carl_m', config=args):
        ...
        wandb.watch(model, torch.nn.functional.mse_loss(), log="all", log_freq=20)
        ...
        for e in range(config.epochs):
            ...
            for i in range(config.steps_per_epoch):
                ...
                if i % config.cb_freq == 0:
                    ...
                    wandb.log({
                        "stft" : utilities.stft_ae_image_callback(y_stft_batch[0], yh_stft[0]),
                        "phasegram" : utilities.video_phasegram_image(y_pgram_batch[0], yh_pgram[0], attn_batch[0]),
                        "audio_input" : dataset.istft(y_stft_batch[0].cpu().detach(), sample_rate=16000),
                        "audio_output" : dataset.istft(yh_stft[0].cpu().detach(), sample_rate=16000)
                    })
