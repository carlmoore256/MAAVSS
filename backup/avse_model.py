...
class AV_Fusion_Model(nn.Module):
    ...
    def __init__(self, 
        ...
        print(f'PHASEGRAM ENCODED SH {tensor.shape}')
        ############### PHASEGRAM DECODER #################
        modules = []
        ...
        print(f'X AV CAT SH {x_av_cat.shape}')
        self.lstm = nn.LSTM(input_size=x_av_cat.shape[-1], hidden_size=256, 
                            num_layers=1, bias=False, batch_first=True, 
                            dropout=0, bidirectional=True).to("cuda")
        ...
        with torch.no_grad():
            x_av_fused = self.av_fusion_forward(x_v, x_a)
