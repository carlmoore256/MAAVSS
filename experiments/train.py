...
if __name__ == "__main__":
    ...
    with wandb.init(project='AV-Fusion-AVSE', entity='carl_m', config=args):
      ...
      pgram_full = utilities.video_phasegram(attn_ex, resize=(config.p_size, config.p_size)).to(DEVICE)
      ...
      for e in range(config.epochs):
          ...
          for i in range(config.steps_per_epoch):
              ...
              attn = attn.to(DEVICE)
              pgram = utilities.video_phasegram(attn, resize=(config.p_size, config.p_size))
              ...
              if i % config.cb_freq == 0:
                  ...
                  output_pgram = torch.zeros_like(pgram_full[:, :, idx_middle_frame:idx_middle_frame+config.num_seq, :])

              for j in range(config.num_seq):
                  if train_mode == 0:
                      x_pgram_batch = pgram_zeros
                  else:
                      x_pgram_batch = pgram[:, :, j:j+config.num_frames, :]
                  y_pgram_batch = pgram[:, :, j+idx_middle_frame, :]
                  ...
                  # print(f'yh stft sh {yh_stft.shape} xstft {x_stft_batch.shape}')
              ...
              if i % config.cb_freq == 0:
                  ...
                  wandb.log({
                      ...
                      "phasegram" : utilities.video_phasegram_image(pgram[0], output_pgram[0], attn[0]),
