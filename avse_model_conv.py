...
class AV_Fusion_Model(nn.Module):
    ...
    def __init__(self, 
        ...
        self.a_fc1 = nn.Sequential(
          nn.Linear(fc2_out, 2 * stft_shape[-2] * stft_shape[-1], bias=False),
          # nn.LeakyReLU(negative_slope=0.3)
          # nn.Tanh()
        )

        self.v_fc1 = nn.Sequential(
          nn.Linear(fc2_out, pgram_shape[-2] * pgram_shape[-1], bias=False),
          # nn.LeakyReLU(negative_slope=0.3)
          # nn.Tanh()
        )
    ...
    def forward(self, x_a, x_v):
        ...
        # print(x_v_out.shape, x_a_out.shape, x_v_enc.shape, x_a_enc.shape)

        x_v_out = x_av_fused.view(x_v_enc.shape)
        x_a_out = x_av_fused.view(x_a_enc.shape)
